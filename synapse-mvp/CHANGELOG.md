...
- Separate **Listen** screen (header Studio / Listen): centered vertical list with now playing, large play/pause, song controls, audio visualizer, upcoming songs, and split branches. Click a song or branch to start playback from that node. While a sequence or conditional branch is already entered, **Leave branch** skips the rest of that arm and continues after the split. The Studio canvas stays the graph editor. Wide windows place player controls on the left and the Music Path on the right; narrow windows keep controls stacked above the path.
